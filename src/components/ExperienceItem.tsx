...
export const ExperienceItem = ({ experience }: Experience) => {
  ...
  return (
    <motion.div
      ref={ref}
      className="flex flex-col gap-8 w-full max-w-screen-lg text-white   shadow-lg rounded-2xl text-[16px] pl-6 pr-1 xl:pl-0 xl:pr-0"
      initial="hidden"
      animate={isInView ? "visible" : "hidden"} // Trigger animation based on scroll
      variants={fadeInUp}
    >
      <div className="flex flex-col w-full bg-[#1a1a1a] border border-emerald-400 rounded-2xl p-6 shadow-md ml-[-5px] mt-[-5px] mb-[5px] z-50">
        {/* Title and Company */}
        <motion.div
          className="flex flex-col xs:flex-row justify-between items-start mb-6"
          variants={fadeInUp}
        >
          {/* Image and Company */}
        ...
        <motion.ol
          className="list-disc ml-6 space-y-2 text-white"
